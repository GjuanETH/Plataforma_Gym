...
const inputStyle = {
    padding: '12px', borderRadius: '8px', border: '1px solid #333', background: '#1a1a1a', color: 'white', width: '100%', outline: 'none', transition: 'border-color 0.3s'
};

export default function CheckoutPage({ onNavigate, user }) {
    const [cart, setCart] = useState([]);
    const [formData, setFormData] = useState({
        firstName: user?.firstName || '', lastName: '', email: '', phone: '', address: '', city: '', zip: '', cardName: '', cardNumber: '', expDate: '', cvv: ''
    });
    ...
    const handlePayment = (e) => {
        ...
        setTimeout(() => {
            ...
            const newOrder = {
                ...
                shipping: { address: formData.address, city: formData.city }
            };

            // Clave única por usuario: gymfit_orders_USERID
            const orderKey = `gymfit_orders_${user.userId}`;
            const currentOrders = JSON.parse(localStorage.getItem(orderKey) || '[]');
            currentOrders.unshift(newOrder); // Agregar al principio
            localStorage.setItem(orderKey, JSON.stringify(currentOrders));

            setIsProcessing(false);
            alert("¡Pago Exitoso! Puedes ver tu pedido en el Dashboard.");
    ...
    if (cart.length === 0) {
        return (
            <div style={{minHeight:'100vh', background:'#0a0a0a', color:'white', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center'}}>
                ...
                <button onClick={() => onNavigate('store')} className="btn-primary" style={{marginTop:'20px', padding:'10px 20px', background:'#E50914', color:'white', border:'none', borderRadius:'5px', cursor:'pointer'}}>Volver a la Tienda</button>
            </div>
        );
    }
    ...
    return (
        <div className="fade-in" style={{minHeight: '100vh', background: '#0a0a0a', color: 'white', fontFamily: 'sans-serif'}}>
            ...
            <div style={{maxWidth: '1100px', margin: '40px auto', padding: '0 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '40px'}}>
                <div>
                    <h2 style={{display:'flex', alignItems:'center', gap:'10px', borderBottom:'1px solid #333', paddingBottom:'15px', marginTop:0}}><MapPin color="#E50914"/> Datos de Envío</h2>
                    <form id="checkout-form" onSubmit={handlePayment} style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginTop:'20px'}}>
                        <input required name="firstName" placeholder="Nombre" value={formData.firstName} onChange={handleInputChange} style={inputStyle} />
                        ...
                        <h2 style={{gridColumn:'span 2', display:'flex', alignItems:'center', gap:'10px', borderBottom:'1px solid #333', paddingBottom:'15px', marginTop:'30px'}}><CreditCard color="#E50914"/> Método de Pago</h2>
                        ...
                        <div style={{maxHeight:'300px', overflowY:'auto', marginBottom:'20px', paddingRight:'5px'}}>
                            {cart.map((item, idx) => (
                                <div key={idx} style={{display:'flex', gap:'15px', marginBottom:'15px', borderBottom:'1px solid #222', paddingBottom:'15px'}}>
                                    <div style={{width:'50px', height:'50px', background:'#fff', borderRadius:'5px', padding:'2px', display:'flex', alignItems:'center', justifyContent:'center'}}><img src={item.image} style={{maxWidth:'100%', maxHeight:'100%', objectFit:'contain'}} alt={item.name}/></div>
                                    <div style={{flex:1}}><div style={{fontSize:'14px', fontWeight:'bold', lineHeight:'1.2'}}>{item.name}</div><div style={{fontSize:'12px', color:'#888', marginTop:'4px'}}>Cant: {item.qty}</div></div>
                                    <div style={{fontWeight:'bold', color:'#E50914'}}>${(item.price * item.qty).toLocaleString()}</div>
                                </div>
                            ))}
                        </div>
                        <div style={{paddingTop:'5px'}}>
                            <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}><span style={{color:'#888'}}>Subtotal</span><span>${total.toLocaleString()}</span></div>
                            <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}><span style={{color:'#888'}}>Envío <Truck size={14} style={{display:'inline', marginLeft:'5px'}}/></span><span style={{color:'#0f0'}}>Gratis</span></div>
                            <div style={{display:'flex', justifyContent:'space-between', fontSize:'22px', fontWeight:'900', marginTop:'20px', borderTop:'1px solid #333', paddingTop:'15px'}}><span>Total</span><span>${total.toLocaleString()}</span></div>
                        </div>
                        <button type="submit" form="checkout-form" disabled={isProcessing} style={{width: '100%', padding: '15px', marginTop: '25px', background: isProcessing ? '#555' : '#E50914', color: 'white', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: 'bold', cursor: isProcessing ? 'not-allowed' : 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', transition: 'background 0.3s'}}>
                            {isProcessing ? 'Procesando...' : <><Lock size={18}/> Pagar Ahora</>}
                        </button>
                        <div style={{textAlign:'center', marginTop:'15px', fontSize:'12px', color:'#666', display:'flex', alignItems:'center', justifyContent:'center', gap:'5px'}}><CheckCircle size={12} color="#0f0"/> Transacción encriptada SSL 256-bit</div>
                    </div>
                </div>
            </div>
        </div>
